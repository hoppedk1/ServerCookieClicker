...
// insert new coins
app.post("/Coinsscore", (request, response) => {
	console.log("Inserting Coinsscore: " + request.body.name + ", " + request.body.coins);

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    var myobj = { name: request.body.name, score: request.body.coins};
    dbo.collection("Coinsscore").insertOne(myobj, function(err, res) {
      if (err) throw err;
...
// get specific coins
app.get("/Coinsscore/:name", (request, response) => {
	console.log("Finding Coinsscore... ");

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    dbo.collection("Coinsscore")
    .find({name: request.params.name})
    .toArray(function(err, result) {
      if (err) throw err;
      ...
      console.log(result);
      //response.header("Access-Control-Allow-Origin", "*");
      response.json({Coinsscore: result});
      db.close();
    });
  });
});
...
// get all coins
app.get("/Coinsscore", (request, response) => {
	console.log("Finding Coinsscore... ");

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    dbo.collection("Coinsscore").find({}).toArray(function(err, result) {
      if (err) throw err;
...
// delete coins
app.delete("/Coinsscore", (request, response) => {
	console.log("Deleting Coinsscore: " + request.body.name);

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    var myquery = { name: request.body.name };
    dbo.collection("Coinsscore").deleteOne(myquery, function(err, obj) {
      if (err) throw err;
...
// updating coins
app.put("/Coinsscore", (request, response) => {
	console.log("Updating Coinsscore of " + request.body.name + " to " + request.body.coins);

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    var myquery = { name: request.body.name };
    var newvalues = { $set: {coins: request.body.coins } };
    dbo.collection("Coinsscore").updateOne(myquery, newvalues, function(err, obj) {
      if (err) throw err;

      console.log("Updated coins of: " + request.body.name + " to " + request.body.coins)
      response.header("Access-Control-Allow-Origin", "*");
      response.json({name: request.body.name, coins: request.body.coins});
      db.close();
    });
  });
});
